...
def register_data_samples(clients: List[substra.Client], key_path: Path):
    ...
    assert (
        len(list(data_path.glob("*"))) == N_CENTERS
    ), "There must be one folder in the data folder per center"
    ...
    for client, center_data_path in zip(
        list(clients.values()), list(data_path.glob("*"))
    ):
        ...
        assert (
            center_data_path / "train"
        ).is_dir(), 'There must be a "train" folder in the center folder'
